...
using robotis::medibot::MediBot;
using namespace std::chrono_literals;

MediBot::MediBot(const std::string & usb_port)
: Node("medibot_node", rclcpp::NodeOptions().use_intra_process_comms(true))
{
  RCLCPP_INFO(get_logger(), "Init MediBot Node Main");
...
MediBot::Wheels * MediBot::get_wheels()
{
  return &wheels_;
}

MediBot::Motors * MediBot::get_motors()
{
  return &motors_;
}

void MediBot::init_dynamixel_sdk_wrapper(const std::string & usb_port)
{
...
void MediBot::check_device_status()
{
...
void MediBot::publish_timer(const std::chrono::milliseconds timeout)
{
...
void MediBot::heartbeat_timer(const std::chrono::milliseconds timeout)
{
...
void MediBot::parameter_event_callback()
{
